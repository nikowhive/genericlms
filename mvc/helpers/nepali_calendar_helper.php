...
class NepaliCalenderHelper
{

	static $nepaliMonthNames = ['बैशाख', 'जेठ', 'असार', 'साउन', 'भदौ', 'असोज', 'कार्तिक', 'मङि्सर', 'पुष', 'माघ', 'फागुन', 'चैत'];
	static $nepaliMonthNamesEng = ["Baishakh", "Jestha", "Ashad", "Shrawan", "Bhadra", "Ashwin", "Kartik", "Mangshir", "Poush", "Magh", "Falgun", "Chaitra"];
	static $nepaliDayNames = ['आइतबार', 'सोमबार', 'मङ्गलबार', 'बुधबार', 'बिहिबार', 'शुक्रबार', 'शनिबार'];
	...
	public function getTotalNoOfDaysOfCertainYearAndMonth($year, $month)
	{
		$yearList = $this->yearList;
		try {
			if ($month > 12 or $month < 1 or !(isset($yearList[$year]))) {
				throw new \Exception('Some error on passing month days count');
			}
		} catch (\Throwable $throwable) {
			return $throwable->getMessage();
		}
		return $yearList[$year][$month - 1];
	}
	...
	{
		$dateArray = explode('-', $date);
		$nepaliDate = $this->englishToNepaliDateInEnglishNumber($dateArray);

		return $nepaliDate;
	}

	public function convertAdToBs($dateTimeString)
	{
		$dateTime = explode(' ', $dateTimeString);
		$dateArray = explode('-', $dateTime[0]);
		$time = $dateTime[1];
		$nepaliDate = $this->eng_to_nep((int)$dateArray[0], (int)$dateArray[1], (int)$dateArray[2]);
		return compact('nepaliDate', 'time');
	}
...
//        }
	...
	public function getDateInNepali($date = '')
	{
		$dateArray = $date ? explode('-', $date) : explode('-', date('Y-m-d'));
		$dateInNepali = $this->eng_to_nep((int)$dateArray[0], (int)$dateArray[1], (int)$dateArray[2]);

		return $dateInNepali;
	}

    public function getFormattedDateInNepali($dateAD)
    {
        $dateArray = $this->getDateInNepali($dateAD);
        return sprintf('%s-%s-%s', $dateArray['year'], $dateArray['month'], $dateArray['date']);
	}
	 ...
	 * @return array
	 */
	public function englishToNepaliDateInEnglishNumber($dateArray)
	{
		$nepaliDate = [];
		$convertedDate = $this->eng_to_nep((int)$dateArray[0], (int)$dateArray[1], (int)$dateArray[2]);
	...
	public function getNepaliMahinaName($num)
	{
		return $this->getMahina($num);
	}
