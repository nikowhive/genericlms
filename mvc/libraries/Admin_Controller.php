...
*/
    ...
    public function __construct()
    {
        ...
        $this->data['loginuserID'] = $loginuserID  = $this->session->userdata('loginuserID');
        ...
        // $start_datetime = $loginLog->start_datetime;
        // $this->data['logintime'] = date("h:i:s",strtotime($start_datetime));
		$this->data['loginendtime'] = $loginLog?$loginLog->end_datetime:'';
    ...
    private function _frontendManager( $siteInfo )
    {
        ...
        $exceptionUris = [
            ...
            'signin/index',
            'signin/signout',
			'courses/ajaxCheckIfMeetingRunning',
			'alert/conversation',
			'alert/alert',
			'event/media',
			'notice/media',
			'holiday/media',
			'liveclass/refreshliveclass',
			'alert/conversation_count'
        ];
	...
	public function reportPDF($stylesheet=NULL, $data=NULL, $viewpath= NULL, $mode = 'view', $pagesize = 'a4', $pagetype='portrait', $type=null) {
		$designType = 'LTR';
		...
		if(!empty($stylesheet)) {
			...
			return $this->mhtml2pdf->create($mode, $this->data['panel_title'], $stylesheet, $type);
		} else { 
			return $this->mhtml2pdf->create($mode, $this->data['panel_title'], $type);
		}
	}
	...
	public function menuTree($dataset, $sessionPermission) {
    	...
    	foreach ($dataset as $id=>&$node) {
			if(isset($node['link'])){
			if((isset($node['link']) && $node['link'] == '#') || (isset($sessionPermission[$node['link']]) && $sessionPermission[$node['link']] != "no") ) {
	    		if ($node['parentID'] == 0) {
	    			$tree[$id]=&$node;
	    		} else {
	...
	public function getHolidaysano() {
    	$schoolyearID = $this->data['siteinfos']->school_year;
		$holidays = $this->holiday_m->get_order_by_holiday(array('schoolyearID' => $schoolyearID));
		$allHolidayList = array();
		if(customCompute($holidays)) {
	...
	public function getWeekendDaysano() {
		$date_from = strtotime($this->data['schoolyearobj']->startingdate);
		$date_to = strtotime($this->data['schoolyearobj']->endingdate);
		$oneDay = 60*60*24;
		...
		for($i= $date_from; $i<= $date_to; $i= $i+$oneDay) {
		    if($weekendDay != "") {
		    	foreach($weekendArrays as $weekendValue) {
		            if($weekendValue >= 1 && $weekendValue <= 7) {
		                if(date('l',$i) == $allDays[$weekendValue]) {
		                    $weekendDateArrays[] = date('d-m-Y', $i);
		                }
		            }
