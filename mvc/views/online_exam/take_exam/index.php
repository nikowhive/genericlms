                ...
                <div id="hide-table">
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        ...
                        <tr>
                            ...
                            <th class="col-sm-2"><?=$this->lang->line('take_exam_startdate')?></th>
                            <th class="col-sm-2"><?=$this->lang->line('take_exam_enddate')?></th>
                            <th class="col-sm-2"><?=$this->lang->line('action')?></th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php if(customCompute($onlineExams)) {$i = 0; foreach($onlineExams as $__index => $onlineExam) {
                            if($usertypeID == '3') {
                                if((($student->srclassesID == $onlineExam->classID) || ($onlineExam->classID == '0')) && (($student->srsectionID == $onlineExam->sectionID) || ($onlineExam->sectionID == '0')) && (($student->srstudentgroupID == $onlineExam->studentGroupID) || ($onlineExam->studentGroupID == '0')) && (($onlineExam->subjectID == '0') || (in_array($onlineExam->subjectID, $userSubjectPluck)))) { $i++;
                                    ...
                                    <tr>
                                        ...
                                        <td data-title="<?=$this->lang->line('take_exam_startdate')?>">
                                            <?php echo $onlineExam->formattedStartDateTime; ?>
                                        </td>
                                        <td data-title="<?=$this->lang->line('take_exam_enddate')?>">
                                            <?php echo $onlineExam->formattedEndDateTime; ?>
                                        </td>
...
<script type="application/javascript">
    ...
    function newPopup(url) {
        window.open(url,'_blank',"width=auto,height=auto,toolbar=0,location=0,scrollbars=no");
