    ...
    <div class="col-md-9">
        <div class="container container--sm" id="feeds-data">
            <header class="pg-header">
                <h1 class="pg-title"><?= $feeds ? $this->lang->line('panel_title') : $this->lang->line('no_events') ?></h1>
                <?php
                if (permissionChecker('event_add')) { ?>
                    <div>
                        <a href="<?php echo base_url('event/add') ?>" class="btn btn-success">
            ...
            <?php foreach ($feeds as $feed) { ?>
                <div class="card p-0 card--media" data-type="event">
                                    ...
                                    <li>
                                        <?php if (permissionChecker('event_edit')) { ?>
                                            <div class="onoffswitch-small">
                                                <input type="checkbox" name="course" class="onoffswitch-small-checkbox" id="event<?= $feed->eventID ?>" <?php if ($feed->published == '1') { ?> checked='checked' <?php }
                                                                                                                                                                                                                if ($feed->published == '1') echo "value='2'";
                                                                                                                                                                                                                else echo "value='1'"; ?>>
                                                <label class="onoffswitch-small-label switch" for="event<?= $feed->eventID ?>" eventid="<?php echo $feed->eventID; ?>">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul id="menu2" class="dropdown-menu" aria-labelledby="drop5">
                                        <?php if (permissionChecker('event_view')) { ?>
                                            <li>
                                                <a href="<?= base_url('event/view/' . $feed->eventID) ?>"><i class="fa fa-eye"></i> View</a>
                                            </li>
                                        <?php } ?>
                                        <?php if (permissionChecker('event_edit')) { ?>
                                            <li>
                                                <a href="<?= base_url('event/edit/' . $feed->eventID) ?>"><i class="fa fa-pencil"></i> Edit</a>
                                            </li>

                                        <?php } ?>
                                        <?php if (permissionChecker('event_delete')) { ?>
                                            <li>
                                                <a href="<?= base_url('event/delete/' . $feed->eventID) ?>" onclick="return confirm('Are you sure you want to delete this event?');"><i class="fa fa-trash"></i> Delete</a>
                                            </li>
                                        <?php } ?>
                                    </ul>
                                </div>
                    ...
                    <?php
                    if (count($feed->media) == 1 || $feed->eventphoto != '') {
                        $media1 = ($feed->media) ? $feed->media[0] : '';
                        $photo = ($media1) ? $media1 : $feed->eventphoto;
                        $photourl = '';
                        if ($media1) {
                            $photourl = base_url("uploads/events/" . $photo);
                            //$thumb_photourl = base_url("uploads/events/256/" . $photo);
                            $thumb_photourl = imagelink1($photo,512,"uploads/events");
                        ...
                        <?php if ($userType != 1) { ?>
                            <div class="list-inline">
                                <?php if (isset($feed->is_going)) {
                                    if ($feed->is_going) { ?>
                                        <a href="#" class="btn btn-default btn-sm" id="going-btn<?= $feed->eventID ?>" style="display: none" onclick="handleEvent('<?= $feed->eventID ?>', 1)">
                                        ...
                                        <a href="#" class="btn btn-default btn-sm" id="not-going-btn<?= $feed->eventID ?>" onclick="handleEvent('<?= $feed->eventID ?>', 0)"> <i class="fa fa-close"></i>
                                            Not Going</a>
                                    ...
                                    <?php } else { ?>
                                        <a href="#" class="btn btn-default btn-sm" id="going-btn<?= $feed->eventID ?>" onclick="handleEvent('<?= $feed->eventID ?>', 1)">
                                            <i class="fa fa-check"></i> Going
                                        </a>
                                        ...
                                        <a href="#" class="btn btn-danger disabled btn-sm" id="disabled-not-going-btn<?= $feed->eventID ?>">
                                            <i class="fa fa-close"></i> You are not going</a>
                                        <a href="#" class="btn btn-default btn-sm" id="not-going-btn<?= $feed->eventID ?>" style="display: none" onclick="handleEvent('<?= $feed->eventID ?>', 0)"> <i class="fa fa-close"></i>
                                            Not Going</a>
                                    <?php } ?>
                                <?php } else { ?>
                                    <a href="#" class="btn btn-primary disabled btn-sm" id="disabled-going-btn<?= $feed->eventID ?>" style="display: none">
...
$('.viewMoreComment').click(function(){
    ...
    function handleEvent(eventId, isGoing) {
        const eventObject = {
            id: eventId,
            status: isGoing
        }
        $.ajax({
            ...
            success: function(response) {
                ...
                if (isGoing) {
                    $('#going-btn' + eventId).hide();
                    $('#disabled-going-btn' + eventId).show();
                    $('#not-going-btn' + eventId).show();
                    $('#disabled-not-going-btn' + eventId).hide();
                } else {
                    $('#not-going-btn' + eventId).hide();
                    $('#disabled-not-going-btn' + eventId).show();
                    $('#going-btn' + eventId).show();
                    $('#disabled-going-btn' + eventId).hide();
                }
                const overAllInfo = '<b>' + response.going + '</b> Going - <b>' + response.not_going + '</b> Not Going';
                $('#overall-event-info-' + eventId).html(overAllInfo);
                showSuccessToast();
            }
        });
    }
    ...
    $(document).ready(function() {
        ...
        $('body').scroll(function() {
            if ($('body').scrollTop() + $('body').height() >= $(document).height()) {
                ...
                if (hasData) {
                    $.ajax({
                        url: "<?= base_url('event/getMoreEventData/') ?>" + pageValue,
                        type: "get",
                        success: function(response) {
