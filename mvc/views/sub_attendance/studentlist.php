<style>
 ...
 <div class="container container--sm">
   <header class="pg-header mt-4">
     <h1 class="pg-title">Subject Wise Attendance</h1>
   </header>
     ...
     <div class="col-md-4">
       <div class="md-form-block">
         <div class="md-form input-with-post-icon">
           <input placeholder="Select date" type="text" id="attdate" class="form-control" value="<?php echo isset($attdate) ? $attdate : date('d-m-Y'); ?>" />
         ...
         <div class="md-form--select md-form">
           <?php
            ...
            if (customCompute($subjects)) {
              ...
              $subID = isset($subjectID) ? $subjectID : '';
            }
            echo form_dropdown("subjectID", $subjectArray, set_value("subjectID", $subID), "id='subjectID' class='mdb-select'");
            ?>
         </div>
       </div>
     </div>
 ...
 <script>
   ...
   $(window).on("load", function() {
     ...
     if (parseInt(idclass) && parseInt(idsection) && attdate && parseInt(subjectID)) {
       $.ajax({
         type: 'POST',
         url: "<?= base_url('sub_attendance/take_attendance') ?>",
         data: {
           "id": idclass,
           "attdate": attdate,
           "idsection": idsection,
   ...
   $("#classesID").change(function() {
     ...
     var attdate = $('#attdate').val();
     <?php if ($userType == 1) { ?>
       url = "<?= base_url('sub_attendance/subjectall') ?>";
     <?php } else { ?>
       url = "<?= base_url('sub_attendance/teacher_subjectall') ?>";
     <?php } ?>
       ...
       $.ajax({
         ...
         success: function(data) {
           ...
           $("#subjectID").on('change', function() {
             ...
             $.ajax({
               type: 'POST',
               url: "<?= base_url('sub_attendance/classsectionall') ?>",
               data: {
                 "id": id,
                 "attdate": attdate,
                 "subjectID": subjectID
   ...
   $("#subjectID").change(function() {
     ...
     var attdate = $('#attdate').val();
     var subjectID = $('#subjectID').val();
     window.location.replace("<?php echo base_url('attendance/index') ?>" + "/" + id + "/" + attdate + "/" + subjectID);
   });
   ...
   $("#attdate").change(function() {
     ...
     if (parseInt(id) && parseInt(idsection) && parseInt(subjectid) && attdate) {
       $.ajax({
         type: 'POST',
         url: "<?= base_url('sub_attendance/take_attendance') ?>",
         data: {
   ...
   function addnote() {
     ...
     $.ajax({
       type: 'POST',
       url: "<?= base_url('sub_attendance/addnote') ?>",
       data: {
         "attID": attID,
         "datevalue": datevalue,
         "notes": notes,
   ...
   function saveattendance(elem) {
     ...
     $.ajax({
       type: 'POST',
       url: "<?= base_url('sub_attendance/ajaxBulkattend') ?>",
       data: JSON.stringify({
         "datevalue": attdate,
         "statuses": idarray,
         "attid": attid,
       ...
       success: function(response) {
         ...
         $.ajax({
           type: 'POST',
           url: "<?= base_url('sub_attendance/getpresent') ?>",
           data: {
             "id": classesid,
             "sectionid": sectionid,
             "attdate": attdate
         ...
         $.ajax({
           type: 'POST',
           url: "<?= base_url('sub_attendance/getlate') ?>",
           data: {
             "id": classesid,
             "sectionid": sectionid,
             "attdate": attdate
         ...
         $.ajax({
           type: 'POST',
           url: "<?= base_url('sub_attendance/getabsent') ?>",
           data: {
             "id": classesid,
             "sectionid": sectionid,
             "attdate": attdate
