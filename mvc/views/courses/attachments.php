            ...
            <div class="container container--sm">
                ...
                <header class="pg-header mt-4">
                    ...
                    <?php if (permissionChecker('attachment_add')) : ?>
                        <a href="javascript:;" data-toggle="modal" data-target="#addAttachment" class="btn-sm btn btn-success"><i class="fa fa-plus"></i> Add attachment</a>
                    <?php endif; ?>
                </header>
                                ...
                                <div class="sortable-block sortable-blockunit">
                                    <div class="sortable-header">
                                        ...
                                        <div class="panned-icon">
                                            <i class='fa <?= checkFileExtension($attachment->attachment) ?>' aria-hidden='true'></i>
                                        </div>
                                        <h3 class="view-attachment sortable-title" data-id="<?= $attachment->id ?>" style="cursor: pointer;" >
                                            <small>
                                                <?php echo $attachment->unit; ?> - <?php echo $attachment->chapter_name; ?></small>
                                    ...
                                    <div class="sortable-actions">
                                            ...
                                            <?php if ($usertypeID == 1 || $usertypeID == 2) : ?>
                                                <label class="switch" data-toggle="tooltip" data-placement="top" data-original-title="Publish/Unpublish">
                                                    <input type="checkbox" class="switch__input" onclick="changeAttachmentStatus('<?= $attachment->id ?>')" class="onoffswitch-small-checkbox" id="switch-attachment<?= $attachment->unit_id ?>" <?= $attachment->published == '1' ? "checked='checked'" : ''; ?>>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <ul id="menu2" class="dropdown-menu" aria-labelledby="drop5">
                                                    <?php if (permissionChecker('attachment_edit')) : ?>
                                                        <li>
                                                            <a href="<?php echo base_url() . 'courses/edit_attachment/' . $attachment->id . '?course=' . $course->id . '&link=' . 'attachments' ?>">Edit Attachment</a>
                                                        </li>
                                                    <?php endif; ?>
                                                    <?php if (permissionChecker('attachment_delete')) : ?>
                                                        <li>
                                                            <a onclick="return confirm('you are about to delete a record. This cannot be undone. are you sure?')" href="<?php echo base_url() . 'courses/deletefile/' . $attachment->id . '?course=' . $course->id . '&link=' . 'attachments' ?>">Delete Attachment</a>
                                                        </li>
                                                    <?php endif; ?>
                                                </ul>
                                            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Add new attachment</h3>
            </div>
...
<script>
    $('.attachment-switch').click(function(e) {
        attachmentid = $(this).attr("attachmentid")
        $.ajax({
            type: 'POST',
            url: "<?= base_url('courses/ajaxChangeFileStatus/') ?>" + attachmentid,
            dataType: "html",
    ...
    $('#add-attachment').click(function(e) {
        ...
        window.location.href = "<?php echo base_url('courses/addfiles/'); ?>" + chapter_id + "?course=" + <?php echo $course->id ?> + "&link=attachments"
        // }
    })
</script>
...
<?php if ($usertypeID == 1 || $usertypeID == 2) : ?>
    <script>
        let url = "<?= base_url('courses/ajaxChangeFileStatus/') ?>";

        function changeAttachmentStatus(id) {
