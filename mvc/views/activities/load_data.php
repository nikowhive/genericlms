        ...
        <div class="box box-widget" style="margin-bottom: 25px;">
            <div class="box-header with-border social-media">
                <div class="user-block">
                    <img class="img-circle" src="<?=isset($user[$activity->usertypeID][$activity->userID]) ?  imagelink($user[$activity->usertypeID][$activity->userID]->photo,56) : imagelink('default.png')?>" alt="User Image">
                ...
                <div class="box-footer box-comments">
                    <?php foreach ($activitiescomments[$activity->activitiesID] as $comment) { if(isset($user[$comment->usertypeID][$comment->userID])) { ?>
                        <div class="box-comment">
                            <img class="img-circle img-sm" src="<?=imagelink($user[$comment->usertypeID][$comment->userID]->photo,56)?>">
            ...
            <?php if(($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) { ?>
                <div class="box-footer">
                    <form action="<?=base_url('activities/index/'.$activity->activitiesID)?>" method="post">
                        <img class="img-responsive img-circle img-sm" src="<?=imagelink($this->session->userdata('photo'),56)?>">
                        <div class="img-push">
