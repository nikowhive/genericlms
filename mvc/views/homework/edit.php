...
<div class="right-side--fullHeight  ">
    ...
    <div class="row w-100 ">
        ...
        <div class="course-content">
            <div class="container container--sm">
                ...
                <h3 class="box-title"><i class="fa icon-homework"></i> <?= $this->lang->line('panel_title') ?></h3>
                    ...
                    <div class="card-body">
                        ...
                        <form class="" enctype="multipart/form-data" role="form" method="post">
                            ...
                            <div class="md-form">
                                <label for="title">
                                    <?= $this->lang->line("homework_title") ?> <span class="text-red">*</span>
                                </label>
                                <input type="text" class="form-control" id="title" name="title" value="<?= set_value('title', $homework->title) ?>">
                    ...
                    <label for="description" class="mdb-main-label">
                        <?= $this->lang->line("homework_description") ?> <span class="text-red">*</span>
                    </label>

                    <div class="md-form">
                        <textarea class="md-textarea form-control" id="description" name="description" rows="4"><?= set_value('description', $homework->description) ?></textarea>
                ...
                <div class="md-form">
                    <label for="deadlinedate">
                        <?= $this->lang->line("homework_deadlinedate") ?> <span class="text-red">*</span>
                    </label>
                    <input type="text" class="form-control" id="deadlinedate" name="deadlinedate" value="<?= set_value('deadlinedate', date('d-m-Y', strtotime($homework->deadlinedate))) ?>">
                    <span class="text-danger error">
            ...
            <div class="md-form--select md-form">
                <?php
                $array = array();
                $array[0] = $this->lang->line("homework_select_classes");
                foreach ($classes as $classa) {
                    $array[$classa->classesID] = $classa->classes;
                }
                echo form_dropdown("classesID", $array, set_value("classesID", $homework->classesID), "id='classesID' class='mdb-select'");
                ?>
                <label for="classesID" class="mdb-main-label">
                    <?= $this->lang->line("homework_classes") ?> <span class="text-red">*</span>
                </label>
                <span class="text-danger error">
        ...
        <div class="md-form--select md-form">
            ...
            <label for="sectionID" class="mdb-main-label">
                <?= $this->lang->line("homework_section") ?>
            </label>
            <span class="text-danger error">
    ...
    <div class="md-form--select md-form">
        <?php
        $array = array('0' => $this->lang->line("homework_select_subject"));
        if ($subjects != "empty") {
            foreach ($subjects as $subject) {
                $array[$subject->subjectID] = $subject->subject;
            }
        }
        echo form_dropdown("subjectID", $array, set_value("subjectID", $homework->subjectID), "id='subjectID' class='mdb-select' disabled");
        ?>
        <label for="subjectID" class="mdb-main-label">
            <?= $this->lang->line("homework_subject") ?> <span class="text-red">*</span>
        </label>
        <span class="text-danger error">
            <?php echo form_error('subjectID'); ?>
        </span>
        <input type="hidden" name="subjectID" value="<?= $homework->subjectID ?>">
    </div>
...
<div class="md-form--select md-form">
    <?php
    $unitArray = array('0' => $this->lang->line("homework_select_unit"));
    if ($units != "empty") {
        foreach ($units as $unit) {
            $unitArray[$unit->id] = $unit->unit_name;
        }
    }
    echo form_dropdown("unitId", $unitArray, set_value("unitId", $homework->unit_id), "id='unitId' class='mdb-select'");
    ?>
    <label for="unitId" class="mdb-main-label">
        <?= $this->lang->line("homework_unit") ?>
    </label>
    <span class="text-danger error">
...
<div class="md-form--select md-form">
    ...
    <?php
    $chapterArray = array('0' => $this->lang->line("homework_select_chapter"));
    if ($chapters != "empty") {
    ...
    echo form_dropdown(
        "chapterId",
        $chapterArray,
        set_value("chapterId", $homework->chapter_id),
        "id='chapterId' class='mdb-select'"
    );
    ?>
    <label for="chapterId" class="mdb-main-label">
        <?= $this->lang->line("homework_chapter") ?>
    </label>
    <span class="text-danger error">
...
<div class="content">
    ...
    <div class="row">
        <?php foreach ($homework_medias as $v) : ?>
...
<input type="submit" class="btn btn-success" value="<?= $this->lang->line("update_homework") ?>">
...
<script>
    ...
    $('#classesID').change(function(event) {
        ...
        if (classesID === '0') {
            ...
            $.ajax({
                type: 'POST',
                url: "<?= base_url('homework/subjectcall') ?>",
                data: "id=" + classesID,
                dataType: "html",
            ...
            $.ajax({
                type: 'POST',
                url: "<?= base_url('homework/sectioncall') ?>",
                data: "id=" + classesID,
                dataType: "html",
                success: function(data) {
                    $('#sectionID').html(data);
    ...
    $('#unitId').change(function(event) {
        ...
        if (unitId === '0') {
            ...
            $.ajax({
                type: 'GET',
                url: "<?= base_url('homework/getChapters/') ?>" + unitId,
                dataType: "html",
                success: function(data) {
                    $('#chapterId').html('');
                    $('#chapterId').html(data);
    ...
    $(function() {
        ...
        $('.image-preview-clear').click(function() {
            ...
            $(".image-preview-input-title").text("<?= $this->lang->line('homework_file_browse') ?>");
        });
    ...
    function deleteImage(id) {
        ...
        if (result) {
            $.post("<?php echo base_url('homework/deleteImage'); ?>", {
                id: id
            }, function(data) {
