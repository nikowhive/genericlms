                    ...
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        <thead>
                            <tr>
                                ...
                                <th class="col-lg-2"><?=$this->lang->line('exam_is_final_term')?></th>
                        ...
                        <tbody>
                            <?php if(customCompute($exams)) {$i = 1; foreach($exams as $exam) { ?>
                                <tr>
                                    ...
                                    <td data-title="<?=$this->lang->line('exam_note')?>">
                                        <span class="label label-primary"><?php echo $exam->is_final_term == 1?'Yes':'No' ?></span>
                                    </td>
                                  
                                    <?php if(permissionChecker('exam_edit') || permissionChecker('exam_delete')) { ?>
                                    <td data-title="<?=$this->lang->line('action')?>">
                                        <?=btn_edit('exam/edit/'.$exam->examID, $this->lang->line('edit')) ?>
                                        <form method="post" action="<?php echo base_url() ?>exam/postChangeExamStatus/<?php echo $exam->examID; ?>" style="text-align: -webkit-center;">
                                            <div class="onoffswitch-small" data-placement="top" data-toggle="tooltip" data-original-title="Publish Result" style ="margin-top: -22px; height: 20px; text-align: -webkit-center;">
                                                <input type="checkbox"  class="onoffswitch-small-checkbox" name="published" <?php if($exam->published == '1') { ?> checked='checked' <?php } if($exam->published == '1')  echo "value='2'";  else echo "value='1'"; ?>>
