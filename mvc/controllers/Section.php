	...
	protected function rules() {
		$rules = array(
			array(
				...
				'rules' => 'trim|required|xss_clean|callback_unique_section'
			),
			array(
				'field' => 'category',
			...
			array(
				...
				'rules' => 'trim|required|max_length[11]|xss_clean|numeric|is_natural|callback_valid_number'
			),
			array(
				'field' => 'classesID',
	...
	public function edit() {
		...
		if((int)$id && (int)$url) {
			...
			if(isset($fetchClass[$url])) {
				if(customCompute($this->data['section'])) {
					$this->data['set'] = $url;
					if($_POST) {
						$rules = $this->rules();
						$this->form_validation->set_rules($rules);
