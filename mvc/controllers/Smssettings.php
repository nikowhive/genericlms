...
*/
	...
	protected function rules_sparrow() {
		$rules = array(
			array(
				'field' => 'sparrow_url',
				'label' => $this->lang->line("smssettings_sparrow_url"),
				'rules' => 'trim|xss_clean|max_length[255]|callback_unique_field'
			),
			array(
				'field' => 'sparrow_from',
				'label' => $this->lang->line("smssettings_sparrow_from"),
				'rules' => 'trim|xss_clean|max_length[255]|callback_unique_field'
			),
			array(
				'field' => 'sparrow_token',
				'label' => $this->lang->line("smssettings_sparrow_token"),
				'rules' => 'trim|xss_clean|max_length[255]|callback_unique_field'
			),
		);
		return $rules;
	...
	public function unique_field($field) {
        if($this->input->post('type') == 'clickatell') {
            ...
            return TRUE;
        } elseif($this->input->post('type') == 'sparrow') {
        	if(!empty($this->input->post('sparrow_url')) || !empty($this->input->post('sparrow_from')) || !empty($this->input->post('sparrow_token')) ) {

        		if($this->input->post('sparrow_url') == $field) {
            		if($this->input->post('sparrow_url') == '') {
	            		$this->form_validation->set_message("unique_field", "The %s is required.");
	                	return FALSE;
            		}
            		return TRUE;
            	...
            	if($this->input->post('sparrow_from') == $field) {
            		if($this->input->post('sparrow_from') == '') {
	            		$this->form_validation->set_message("unique_field", "The %s is required.");
	                	return FALSE;
            		}
            		return TRUE;
				...
				if($this->input->post('sparrow_token') == $field) {
            		if($this->input->post('sparrow_token') == '') {
	            		$this->form_validation->set_message("unique_field", "The %s is required.");
	                	return FALSE;
            		}
            		return TRUE;
	...
	public function index() {
		...
		$sparrow_bind = array();
		$get_sparrows = $this->smssettings_m->get_order_by_sparrow();
		foreach ($get_sparrows as $key => $get_sparrow) {
			$sparrow_bind[$get_sparrow->field_names] = $get_sparrow->field_values;
		}
		$this->data['set_sparrow'] = $sparrow_bind;
		...
		if($_POST) {
			...
			if($type == 'clickatell') {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $url = $this->input->post('sparrow_url');
                    $from = $this->input->post('sparrow_from');
					$token = $this->input->post('sparrow_token');

                    $array = array(
                        array(
                            'field_names' => 'sparrow_url',
                            'field_values' => $url
                        ),
                        array(
                            'field_names' => 'sparrow_from',
                            'field_values' => $from
						),
						array(
                            'field_names' => 'sparrow_token',
                            'field_values' => $token
                        )
                    );

                    $this->smssettings_m->update_sparrow($array);
                    $this->data["subview"] = "smssettings/index";
                    $this->load->view('_layout_main', $this->data);
                }
            } 
