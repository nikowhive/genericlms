...
*/
	...
	private function getView($getUser) {
		if(customCompute($getUser)) {
			...
			if($getUser->usertypeID == 3) {
				...
				$this->linegraphinfo($getUser);
				$this->bargraphinfo($getUser);
			}
	...
	private function linegraphinfo($getuser){
		
        $this->db->select('subjectName,studentName,roll_no,classesName,sectionName');
        $this->db->from('marksentries');
        $this->db->where(['studentID'=>$getuser->srstudentID,'schoolyearID'=>$getuser->srschoolyearID]);
        ...
        if($outputs){
            ...
            $this->data['exams'] = json_encode($exams);
            $this->data['gpaarray1']=json_encode($gpaarray1);
	    }    
	}

	public function bargraphinfo($getuser){
        $this->db->select('subjectName,studentName,roll_no,classesName,sectionName');
        $this->db->from('marksentries');
        $this->db->where(['studentID'=>$getuser->srstudentID,'schoolyearID'=>$getuser->srschoolyearID]);
        $this->db->order_by('marksentriesID','ASC');
        ...
        if($outputs){
	        ...
	        foreach($outputexams as $outputexam){
	            $this->db->select('gpa');
	            $this->db->from('marksentries');
	            $this->db->where(['studentID'=>$getuser->srstudentID,'schoolyearID'=>$getuser->srschoolyearID,'examName'=>$outputexam->examName]);
	            $this->db->order_by('marksentriesID','ASC');
	        ...
	        $this->data['gpaarray1']=json_encode($gpaarray);
	    } else {
            $subject = [];
            $this->data['subject'] = json_encode($subject);
            $this->data['studentName'] = '';
