...
*/
    ...
    protected function rules()
    {
        $rules = [
            ...
            [
                'field' => 'principal',
                'label' => $this->lang->line("setting_school_photo"),
                'rules' => 'trim|max_length[200]|xss_clean|callback_principalupload'
            ],
            [
                'field' => 'enable_principal_signature',
                'label' => $this->lang->line("setting_school_photo"),
            ...
            [
                'field' => 'schoolDays',
                'label' => $this->lang->line("setting_school_days"),
                'rules' => 'trim|required|xss_clean|max_length[128]'
            ],
            [
                'field' => 'principal_name',
                'label' => $this->lang->line("setting_principal_name"),
                'rules' => 'trim|required|xss_clean|max_length[128]'
            ],
            [
                'field' => 'vice_principal_name',
                'label' => $this->lang->line("setting_vice_principal_name"),
                'rules' => 'trim|required|xss_clean|max_length[128]'
            ],
        ];
    ...
    public function principalupload()
    {
        $setting  = $this->setting_m->get_setting();
        $new_file = "site.png";
        if ( $_FILES["principal"]['name'] != "" ) {
            $file_name        = $_FILES["principal"]['name'];
            $random           = random19();
            $makeRandom       = hash('sha512', $random . config_item("encryption_key"));
            $file_name_rename = $makeRandom;
            $explode          = explode('.', $file_name);
            if ( customCompute($explode) >= 2 ) {
                ...
                if ( !$this->upload->do_upload("principal") ) {
                    ...
                    $this->upload_data['principal'] = $this->upload->data();
                    return true;
                }
            } else {
                $this->form_validation->set_message("photoupload", "Invalid file");
            ...
            if ( customCompute($setting) ) {
                $this->upload_data['principal'] = [ 'file_name' => $setting->principal ];
                return true;
            } else {
                $this->upload_data['principal'] = [ 'file_name' => $new_file ];
                return true;
            }
        }
    }
        ...
        if ( $this->data['setting'] ) {
            if ( $_POST ) {
                ...
                if ( $this->form_validation->run() == false ) {
                    ...
                    $array['principal']        = $this->upload_data['principal']['file_name'];
