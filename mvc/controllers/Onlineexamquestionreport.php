...
*/
			...
			if($_POST) {
				...
				if($this->form_validation->run() == FALSE) {
					...
					if(customCompute($this->data['exam'])) {
						...
						$this->data['questions'] = pluck($this->question_bank_m->get_question_bank_questionArray($this->data['exam_questions'],'questionBankID'),'obj','questionBankID');

						$this->data['question_options'] = pluck_multi_array($this->question_option_m->get_question_option_by_questionArray($this->data['exam_questions'],'questionID'),'obj','questionID');

						$this->data['getMark'] = $this->getTotalExamMark($this->data['exam_questions'],$this->data['questions']);

						if($typeID == 3) {
							$allAnswers = $this->question_answer_m->get_where_in_question_answer($this->data['exam_questions'],'questionID');
							$answers = [];
	...
	private function getTotalExamMark($examQuestions,$allQuestions) {
		$mark = 0;
		if(customCompute($examQuestions) && customCompute($allQuestions)) {
