...
class Kindergarten extends Admin_Controller {
/*
| -----------------------------------------------------
...
*/
	...
	public function add() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
	        ...
	        $this->data['classes']  = $this->classes_m->get_order_by_classes_only_kg(['classesID !='=> $graduateclass, 'classes like' => '%kg%']);

	        if($_POST) {
	            $rules = $this->rules();
	            $this->form_validation->set_rules($rules);
	            if ($this->form_validation->run() == FALSE) {
	                $this->data["subview"] = "mark1/add";
	                $this->load->view('_layout_main', $this->data);
	            } else {
	...
	private function getMarkPrintPDF($studentID, $classesID) {
		if((int)$studentID && (int)$classesID) {
			...
			if(customCompute($student) && customCompute($classes)) {
				...
				$this->reportPDF('markmodule.css',$this->data, 'mark1/print_preview');
			} else {
				$this->data['settingmarktypeID'] = 0;
				$this->data['subjects']          = [];
				$this->data['exams']             = [];
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if((int)$studentID && (int)$classesID) {
						...
						if(customCompute($student) && customCompute($classes)) {
							...
							$this->reportSendToMail('markmodule.css',$this->data, 'mark1/print_preview', $email, $inputsubject, $message);
	...
	public function mark_list() {
		...
		if((int)$classID) {
			$string = base_url("kindergarten/index/$classID");
			echo $string;
		} else {
			redirect(base_url("kindergarten/index"));
		}
	}
