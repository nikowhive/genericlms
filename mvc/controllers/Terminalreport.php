...
*/
    ...
    public function importReport(){
		...
		$this->data["subview"]  = "report/terminal/ImportView";
		$this->load->view('_layout_main', $this->data);
    }

    public function importNotify(){
		...
		if(permissionChecker('terminalreport')) {
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
                    ...
                    //print_r($this->data['studentLists']);die();
                    foreach($this->data['subjects'] as $subject){
                    	if($subject->type==1){
                    		$marks=$this->data['studentPosition'][$this->data['studentIDD']]['subjectMark'][$subject->subjectID];
                    		if($subject->finalmark==50){
                    			$marks = ($this->data['studentPosition'][$this->data['studentIDD']]['subjectMark'][$subject->subjectID])*2;
                    		}
                    		//print_r($marks);die();
                    		...
                    		$entries = [
	                            'classesID'=>$this->data['studentLists'][0]->srclassesID,
	                            'classesName'=>$this->data['studentLists'][0]->srclasses,
	                            'sectionID'=>$this->data['studentLists'][0]->srsectionID,
	                            'sectionName'=>$this->data['studentLists'][0]->srsectionID,
	                            'examID'=>$this->data['examID'],
	                            'examName'=>$this->data['examName'],
	                            'studentID'=>$this->data['studentIDD'],
	                            'studentName'=>$this->data['studentLists'][0]->srname,
	                            'roll_no'=>$this->data['studentLists'][0]->srroll,
                            ...
                            $array = ['classesID'=>$this->data['studentLists'][0]->srclassesID,'sectionID'=>$this->data['studentLists'][0]->srsectionID,'examID'=>$this->data['examID'],'studentID'=>$this->data['studentIDD'],'subjectID'=>$subject->subjectID,'schoolyearID'=>$schoolyearID];
                            $this->db->select('marksentriesID');
                            $this->db->from('marksentries');
                            $this->db->where($array);
                            $query = $this->db->get();
                            $objectquery = $query->row();
                            if(!$objectquery){
	                            if($this->db->insert('marksentries', $entries)){
	                            	$this->session->set_flashdata('success_message', 'successfully Imported');
	                            } else {
	                            	$this->session->set_flashdata('failure_message', 'Problem in import');
	                            }
                            }
