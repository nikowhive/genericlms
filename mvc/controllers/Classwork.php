...
class Classwork extends Admin_Controller
{
/*
| -----------------------------------------------------
 ...
 */
    ...
    public function fileupload()
    {
        ...
        $classwork = [];
        if ((int) $id) {
            $classwork = $this->classwork_m->get_single_classwork(array('classworkID' => $id));
        }
        ...
        if ($_FILES["file"]['name'] != "") {
            ...
            if (customCompute($classwork)) {
                $this->upload_data['file']                       = array('file_name' => $classwork->file);
                $this->upload_data['file']['original_file_name'] = $classwork->originalfile;
                return true;
            } else {
    ...
    public function index()
    {
        ...
        if ((int) $classesID && isset($fetchClasses[$classesID])) {
            ...
            $this->data['classworks'] = $this->classwork_m->join_get_classwork($classesID, $schoolyearID);

            $this->data["subview"] = "classwork/index";
            $this->load->view('_layout_course', $this->data);
        } else {
            $this->data['set']        = 0;
            $this->data['sections']   = [];
            $this->data['classworks'] = [];
        ...
        if (($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            if ($_POST) {
                ...
                if ($this->form_validation->run() == false) {
                    $this->data["subview"] = "classwork/add";
                    $this->load->view('_layout_course', $this->data);
                } else {
                    $array = array(
                    ...
                    // $this->pushNotification("Classwork for subject " . $subject . " for class " . $class . " has been added", $class);
                    $this->session->set_flashdata('success', $this->lang->line('menu_success'));

                    if ($course == '') {
                        redirect(base_url("classwork/index"));
                    } else {
                        if ($link == 'classwork') {
                            redirect(base_url("courses/classwork/" . $course));
                        } else {
        ...
        if ((int) $id && (int) $url) {
            if (($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
                ...
                if (isset($fetchClasses[$url])) {
                    ...
                    if ($this->data['classwork']) {
                        $this->data['sectionID'] = json_decode($this->data['classwork']->sectionID);

                        if ($this->input->post('classesID')) {
                            $classesID = $this->input->post('classesID');
                        } else {
                            $classesID = $this->data['classwork']->classesID;
                        }
                        ...
                        if ($_POST) {
                            ...
                            $this->form_validation->set_rules($rules);
                            if ($this->form_validation->run() == false) {
                                $this->data["subview"] = "classwork/edit";
                                $this->load->view('_layout_course', $this->data);
                            } else {
                                $array = array(
                                    "title"            => $this->input->post("title"),
                                ...
                                $this->db->trans_start();
                                $this->classwork_m->update_classwork($array, $id);
                                
                                $photos = $this->upload_data['files'];
                                if (customCompute($photos)) {
                                    foreach ($photos as $key => $photo) {
                                        $photos[$key]['classworkID'] = $id;
                                    }
                                    $this->classwork_media_m->insert_batch_classwork_media($photos);
                                }
                                ...
                                if ($course == '') {
                                    redirect(base_url("classwork/index/$url"));
                                } else {
                                    if ($link == 'classwork') {
                                        redirect(base_url("courses/classwork/" . $course));
                                    } else {
    ...
    public function deleteImage()
	{
		if ($this->input->post('id')) {
			...
			$delete = $this->classwork_media_m->delete_classwork_media($id);
                ...
                if (isset($fetchClasses[$url])) {
                    ...
                    if (customCompute($classwork)) {
                        if (config_item('demo') == false) {
                            if ($classwork->file != '') {
                                if (file_exists(FCPATH . 'uploads/images/' . $classwork->file)) {
                                    unlink(FCPATH . 'uploads/images/' . $classwork->file);
                                }
                            }
                        }
                        $this->classwork_m->delete_classwork($id);
                        $this->session->set_flashdata('success', $this->lang->line('menu_success'));
                        if ($course == '') {
                            redirect(base_url("classwork/index/$url"));
                        } else {
    ...
    {
        $classworkID  = htmlentities(escapeString($this->uri->segment(3)));
        ...
        if ((int) $classworkID && (int) ($classesID)) {
            $fetchClasses = pluck($this->classes_m->get_classes(), 'classesID', 'classesID');
            if (isset($fetchClasses[$classesID])) {
                $this->data['viewclass'] = $classesID;
                $classwork               = $this->classwork_m->get_single_classwork(array('classworkID' => $classworkID, 'classesID' => $classesID, 'schoolyearID' => $schoolyearID));
                ...
                $f = true;
                if (($usertypeID == 3) && customCompute($student) && in_array($classwork->subjectID, $opsubjects) && ($student->sroptionalsubjectID != $classwork->subjectID)) {
                    $f = false;
                }

                // log start
                $event = 'classwork submission list';
                $remarks = 'visited classwork: '.$classwork->title .' submission of  '.$this->data['course']->classes . ' - ' . $this->data['course']->subject. ' by '.$this->session->userdata("name");
                createCourseLog($event,$remarks);
                // log end

                if (customCompute($classwork) && $f) {
                    $this->data['classworkanswers'] = $this->classworkanswer_m->join_get_classworkanswer($classworkID, $schoolyearID, $loginuserID);
                    $this->data["subview"]          = "classwork/view";
                    $this->load->view('_layout_course', $this->data);
                } else {
                    $this->data["subview"] = "error";
                    $this->load->view('_layout_course', $this->data);
    ...
    public function classworkanswer()
    {
        $id           = htmlentities(escapeString($this->uri->segment(3)));
        $url          = htmlentities(escapeString($this->uri->segment(4)));
        $usertypeID   = $this->session->userdata('usertypeID');
            ...
            if ((int) $id && (int) ($url)) {
                ...
                if (isset($fetchClasses[$url])) {
                    if ($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) {
                        $classwork = $this->classwork_m->get_single_classwork(array('classworkID' => $id, 'schoolyearID' => $schoolyearID));
                        $classworkanswer = $this->classworkanswer_m->get_single_classworkanswer(array('uploaderID' => $userID, 'uploadertypeID' => $usertypeID, 'schoolyearID' => $schoolyearID, 'classworkID' => $id));
                        $this->data['content'] = $classworkanswer?$classworkanswer->content:'';
                        if($classworkanswer){
                            $this->data['classwork_answer_medias'] = $this->classwork_answer_media_m->get_order_by_classwork_answer_media(['classworkanswerID' => $classworkanswer->classworkanswerID]);
                        }else{
                            $this->data['classwork_answer_medias'] = [];
                        }
                                ...
                                if ($_POST) {
                                    ...
                                    if ($this->form_validation->run() == false) {
                                        ...
                                        if (customCompute($classworkanswer)) {
                                            ...
                                            // log start
                                            $event = 'classwork answer update';
                                            $remarks = 'classwork: '.$classwork->title.' answer submitted by '.$this->session->userdata("name");
                                            createCourseLog($event,$remarks);
                                            // log end
                                            ...
                                            if ($classworkanswerID) {
                                               ...
                                               // log start
                                               $event = 'classwork answer submission';
                                               $remarks = 'classwork: '.$classwork->title.' answer updated by '.$this->session->userdata("name");
                                               createCourseLog($event,$remarks);
                                               // log end
    ...
    public function classworkdownloadFiles()
    {
        $id = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $id) {
           
            $classworkFile = $this->classwork_media_m->get_single_classwork_media(array('id' => $id));
           
            if (customCompute($classworkFile)) {
                $file         = realpath('uploads/images/' . $classworkFile->attachment);
                $originalname = $classworkFile->attachment;
                if (file_exists($file)) {
    ...
    public function answerdownloadFiles()
    {
        ...
        if ((int) $id) {
            ...
            $classworkanswerFile = $this->classwork_answer_media_m->get_single_classwork_answer_media(array('id' => $id));
           
            if (customCompute($classworkanswerFile)) {
                $file         = realpath('uploads/images/' . $classworkanswerFile->attachment);
                $originalname = $classworkanswerFile->attachment;
                if (file_exists($file)) {
                    header('Content-Description: File Transfer');
                    header('Content-Type: application/octet-stream');
                    header('Content-Disposition: attachment; filename="' . basename($originalname) . '"');
    ...
    public function viewClassworkAnswerByAjax(){

        $schoolyearID = $this->session->userdata('defaultschoolyearID');
        $id = $this->input->post('id');
        $this->data['classworkanswer'] = $classworkanswer = $this->classworkanswer_m->get_single_classworkanswer(array('classworkanswerID' => $id));
        $this->data['classwork_answer_medias'] = $this->classwork_answer_media_m->get_order_by_classwork_answer_media(['classworkanswerID' => $classworkanswer->classworkanswerID]);
        ...
        echo $this->load->view('courses/classworkanswerview', $this->data, true);
        exit;
    ...
    public function loadRemarkForm(){
        ...
        $id = $this->input->post('id');
        $this->data['classworkanswer'] = $this->classworkanswer_m->get_single_classworkanswer(array('classworkanswerID' => $id));
        echo $this->load->view('classwork/addAnswerRemark', $this->data, true);
        exit;
    }
    ...
    public function addRemarks(){
        ...
        $schoolyearID = $this->session->userdata('defaultschoolyearID');
        $id = $this->input->post('classworkanswerID');
    ...
    public function updateClassworkStatus(){
        ...
        foreach($idArray as $id){
                ...
                if($returnid = $this->classworkanswer_m->update_classworkanswer($data,$id)){
                    $classworkanswer = $this->classworkanswer_m->get_single_classworkanswer(array('classworkanswerID' => $id));
                    $userID = $classworkanswer->uploaderID;
                    $usertypeID = $classworkanswer->uploadertypeID;
                    $u[] = $userID.$usertypeID; 
    ...
    public function updateSingleClassworkStatus()
    {

        $id = $this->input->get('id');
        $answer = $this->classworkanswer_m->get_classworkanswer($id);
        ...
        $returnid = $this->classworkanswer_m->update_classworkanswer($array, $id);
        if($returnid){

            $classwork = $this->classwork_m->get_single_classwork(['classworkID' => $answer->classworkID]);
            $title = 'Classwork submission checked.';
            $notice = 'Your classwork: ' . $classwork->title . ' has been checked';
    ...
    public function subjectcall()
    {
        ...
        if ((int) $classID) {
            ...
            echo "<option value='0'>", $this->lang->line("classwork_select_subject"), "</option>";
    ...
    public function student_list()
    {
        $classID = $this->input->post('id');
        if ((int) $classID) {
            $string = base_url("classwork/index/$classID");
            echo $string;
        } else {
            redirect(base_url("classwork/index"));
        }
    ...
    public function download()
    {
        ...
        if ((int) $id) {
            ...
            $classwork    = $this->classwork_m->get_single_classwork(array('classworkID' => $id, 'schoolyearID' => $schoolyearID));
            if (customCompute($classwork)) {
                $file         = realpath('uploads/images/' . $classwork->file);
                $originalname = $classwork->originalfile;
                if (file_exists($file)) {
                    header('Content-Description: File Transfer');
                    header('Content-Type: application/octet-stream');
                    header('Content-Disposition: attachment; filename="' . basename($originalname) . '"');
    ...
    public function answerdownload()
    {
        ...
        if ((int) $id) {
            $schoolyearID    = $this->session->userdata('defaultschoolyearID');
            $classworkanswer = $this->classworkanswer_m->get_single_classworkanswer(array('classworkanswerID' => $id, 'schoolyearID' => $schoolyearID));
            if (customCompute($classworkanswer)) {
                $file         = realpath('uploads/images/' . $classworkanswer->answerfile);
                $originalname = $classworkanswer->answerfileoriginal;
                if (file_exists($file)) {
                    header('Content-Description: File Transfer');
                    header('Content-Type: application/octet-stream');
                    header('Content-Disposition: attachment; filename="' . basename($originalname) . '"');
    ...
    public function importAnswer(){
        $classworkanswers = $this->classworkanswer_m->get_order_by_classworkanswer([
            'schoolyearID' => $this->session->userdata('defaultschoolyearID')
        ]);
       
        if(customCompute($classworkanswers)){
            foreach($classworkanswers as $classworkanswer){
                if($classworkanswer->answerfile != ''){
                     $classworkanswerID = $classworkanswer->classworkanswerID;
                     $classworkanswerMedia = $this->classwork_answer_media_m->get_order_by_classwork_answer_media(['classworkanswerID'=> $classworkanswerID]);
                     if(!customCompute($classworkanswerMedia)){
                         $data = [
                             'classworkanswerID' => $classworkanswerID,
                             'attachment'         => $classworkanswer->answerfile,
                             'caption'            => 'caption',
                             'create_date'       => date('Y-m-d H:i:s')
                         ];
                         if($this->classwork_answer_media_m->insert_classwork_answer_media($data)){
                             echo 'success';
                         }else{
                             echo 'error';
                         }
