...
*/
	...
	public function index() {
		...
		if($usertypeID == 3) {
			...
			if($lID != "" || !empty($lID)) {
				...
				$this->data['issues'] = $this->issue_m->get_all_issue_with_books();
				$this->data["subview"] = "issue/index";
				$this->load->view('_layout_main', $this->data);
			}
		}
	...
	protected function rules() {
		$rules = array(
			...
			array(
				...
				'rules' => 'trim|required|xss_clean|max_length[10]|callback_wrong_date'
			),
			array(
				'field' => 'due_date_in_bs', 
				'label' => $this->lang->line("issue_due_date"),
	...
	public function add() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			$this->data['libraries'] = $this->lmember_m->get_lmember();
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$array = array(
						...
						"due_date_in_bs" => $this->input->post("due_date_in_bs"),
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			$this->data['library_member'] = $this->lmember_m->get_single_lmember_with_student(['lID' => $lID]);
			$this->data['books'] = $this->book_m->get_book();
			if((int)$id && $lID) {
			...
			if(customCompute($this->data['book'])) {
				...
				if(customCompute($lmember)) {
					...
					if(customCompute($this->data['student'])) {
						...
						$this->data['invoices'] = $this->invoice_m->get_book_invoice_by_student(
							[
								'studentID' => $this->data['student']->studentID,
								'feetypeID' => 1,
								'schoolyearID' => $this->data['student']->schoolyearID
							]
							);
	...
	public function returnbook() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if(permissionChecker('issue_add') && permissionChecker('issue_edit')) {
				if((int)$id && $lID) {
					$date = $this->convertAdToBs(date("Y-m-d"));
	...
	public function renewbook() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if(permissionChecker('issue_add') && permissionChecker('issue_edit')) {
				if((int)$id && $lID) {
					$date = $this->addWeekToCurrentDay();
					$dateInBS = $this->convertAdToBs($date);
					...
					//	$due_quantity = ($book->due_quantity-1);
						//$this->book_m->update_book(array("due_quantity" => $due_quantity), $dbGet_bookID);
						$this->issue_m->update_issue(array("due_date" => $date,"due_date_in_bs" => $dateInBS), $id);
						$this->session->set_flashdata('success', $this->lang->line('menu_success'));
						if($this->session->userdata('usertypeID') == 4) {
							$lmember = $this->lmember_m->get_single_lmember(array('lID' => $issue->lID));
							redirect(base_url("issue/index/$lmember->studentID"));
	...
	public function studentIDcall() {
		$library_memberID = $this->input->post('library_memberID');
		if($library_memberID) {
			$library_member = $this->lmember_m->get_single_lmember_with_student(['lID' => $library_memberID]);
			$name = $library_member->name;
			$roll = $library_member->roll;
			$class = $library_member->classes;
			$section = $library_member->section;
			$json = array("name" => $name, "roll" => $roll, "class" => $class, "section" => $section);
			header("Content-Type: application/json", true);
			echo json_encode($json);
			exit;
		}
	...
	public function addWeekToCurrentDay(){
		$date = date('Y-m-d');
		$date = strtotime($date);
