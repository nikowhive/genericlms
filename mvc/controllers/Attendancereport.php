...
*/
	...
	public function custom()
	{
		...
		$this->data["subview"] = "report/attendance/AttendanceReportViewc";
		$this->load->view('_layout_main', $this->data);
	}
	...
	protected function rulesc()
	{
		$rules = array(
			...
			array(
				'field' => 'dateto',
				'label' => "Date To",
				'rules' => 'trim|required|xss_clean|callback_valid_date|callback_unique_date|callback_check_holiday|callback_check_weekendday'
			),
		);
		return $rules;
	}
	...
	public function getAttendacneReport()
	{
		...
		if (permissionChecker('attendancereport')) {
			if ($_POST) {
				...
				$subjects =  pluck($this->subject_m->general_get_order_by_subject(['classesID' => $classesID]), 'obj', 'subjectID');
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if ($sectionID == 0) {
						...
						if ($subjectID) {
							if ($subjectID == "all") {
								$attendances = $this->subjectattendance_m->get_order_by_sub_attendance(array('classesID' => $classesID, 'monthyear' => $monthyear, 'schoolyearID' => $schoolyearID));
							} else {
					...
					foreach ($attendances as $index => $attendance) {
						...
						if ($attarray[$day] == 'P' || $attarray[$day] == '') {
							$pcount++;
						}
					...
					if ($subjectID == "all") {
						$attendances = pluck_multi_array($attendances, 'obj', 'studentID');
					} else {
						$attendances = pluck($attendances, 'obj', 'studentID');
					}
					
					$this->data['pcount'] = $pcount;
					$this->data['acount'] = $acount;
					$this->data['studentno'] = $this->student_m->get_student_number($sectionID);
	...
	public function getAttendacneReportForAllClass()
	{
		...
		if (permissionChecker('attendancereport')) {
			if ($_POST) {
				...
				$this->data['typeSortForm'] = $attendancetype;
				$this->data['date'] = $this->input->post('date');
				$this->data['totalStudent'] = $totalStudents;
				$this->data['totalAbsents'] = $totalAbsents;
				$this->data['totalPresents'] = $totalPresents;
				$this->data['totalLeaves'] = $totalLaeves;
				$this->data['results'] = $results;
				$retArray['render'] = $this->load->view('report/attendance/AttendanceReportAllClass.php', $this->data, TRUE);
				$retArray['status'] = TRUE;
				echo json_encode($retArray);
				exit;
	...
	public function getAttendacnecReport()
	{
		$retArray['status'] = FALSE;
		$retArray['render'] = '';
		// if (permissionChecker('attendancereport')) {
		if ($_POST) {
			...
			$subjectID	    = $this->input->post('subjectID');
			$schoolyearID 	= $this->session->userdata('defaultschoolyearID');
			//print_r($this->data['siteinfos']->attendance);die();
			$rules = $this->rulesc($this->data['siteinfos']->attendance);
			$this->form_validation->set_rules($rules);
			if ($this->form_validation->run() == FALSE) {
				$retArray = $this->form_validation->error_array();
				$retArray['status'] = FALSE;
				...
				$monthyearfrom = $datefrom[1] . '-' . $datefrom[2];
				$monthyearto = $dateto[1] . '-' . $dateto[2];
				...
				if ($sectionID == 0) {
					...
					$attendances = $this->sattendance_m->get_order_by_attendancerange($classesID, $sectionID, $monthyearfrom, $monthyearto, $schoolyearID);
				} else {
					$students = pluck($this->studentrelation_m->general_get_order_by_student(array('srclassesID' => $classesID, 'srsectionID' => $sectionID, 'srschoolyearID' => $schoolyearID)), 'obj', 'srstudentID');
					$sections 	= pluck($this->section_m->general_get_order_by_section(array('classesID' => $classesID, 'sectionID' => $sectionID)), 'obj', 'sectionID');
				...
				$attendanceforms = [];
				$j = 0;
				$monthdiff = (($dateto[2] - $datefrom[2]) * 12) + ($dateto[1] - $datefrom[1]);

				foreach ($attendances as $attendance) {

					$attendanceforms[$j]['studentID'] = $attendance['studentID'];
					$attendanceforms[$j]['classesID'] = $attendance['classesID'];
					$attendanceforms[$j]['sectionID'] = $attendance['sectionID'];

					$i = 1;
					for ($k = $datefrom[0]; $k <= ((31 - $datefrom[0]) + ($monthdiff - 1) * 31 + $dateto[0]); $k++) {
						//$dayfrom = 'a' . $k;
						$attendanceforms[$j][$k] = $attendance['a' . $i];
						if ($this->sattendance_m->isMultipleOf31($k)) {
							$attendanceforms[$j][$k] = $attendance['a31'];
							$i = 0;
						}
				...
				$retArray['render'] = $this->load->view('report/attendance/AttendanceReportc', $this->data, TRUE);
				$retArray['status'] = TRUE;
				echo json_encode($retArray);
				exit;
			}
	...
	public function pdfAllClass()
	{
		if (permissionChecker('attendancereport')) {

			$attendancetype = htmlentities(escapeString($this->uri->segment(3)));
			$dateString = date('d-m-Y', htmlentities(escapeString($this->uri->segment(4))));
			$date  = explode('-', $dateString);
			$schoolyearID 	= $this->session->userdata('defaultschoolyearID');
			$this->data['classes']  = $classes = $this->classes_m->general_get_classes();
			...
			if(customCompute($classes)){
				...
				$this->data['totalLeaves'] = $totalLaeves;
				$this->data['results'] = $results;
				$this->reportPDF('attendancereport.css', $this->data, 'report/attendance/AttendanceReportAllClassPDF');
	...
	public function getSubject()
	{
		...
		if ((int)$classID) {
			...
			echo "<option value='all'>", $this->lang->line("attendancereport_select_all"), "</option>";
