				...
				if ($this->form_validation->run() == FALSE) {
					...
					if (customCompute($students)) {
						...
						foreach ($students as $index => $student) {
							...
							$this->data['attendancesID'] = $this->subjectattendance_m->get_single_sub_attendance(array('classesID' => $classesID, 'sectionID' => $sectionID, 'subjectID' => $subjectID, 'schoolyearID' => $schoolyearID, 'studentID' => $student->studentID, 'monthyear' => $monthyear));
							if($this->data['attendancesID']){
							$a = 'a' . $explode_date[0];
							$this->db->select('a' . $explode_date[0]);
							$this->db->from('sub_attendance_note');
							$this->db->where('attendanceID', $this->data['attendancesID']->attendanceID);
							$query = $this->db->get();
							$notesarray = $query->row_array();
							$note=isset($notesarray[$a]) && !empty($notesarray[$a])?$notesarray[$a]:'';
							$students[$index]->note = $this->data['attendancesID']->$a == "P" ? '' : $note;
							}
						}
