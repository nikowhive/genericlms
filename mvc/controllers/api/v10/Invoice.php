    ...
    public function index_get() 
    {
        ...
        if($usertypeID == 3) {
            ...
            if(customCompute($student)) {
                $maininvoices = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_studentID($student->studentID, $schoolyearID);
                foreach($maininvoices as $main)
                {
                    $invoices[$main->maininvoiceID] = $this->invoice_m->get_order_by_invoice(array('maininvoiceID' => $main->maininvoiceID));
                    $main->invoice = $invoices[$main->maininvoiceID];
                }
                $this->retdata['maininvoices'] = $maininvoices;
            ...
            if(customCompute($students)) {
                ...
                $maininvoices = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_multi_studentID($studentArray, $schoolyearID);
                foreach($maininvoices as $main)
                {
                    $invoices[$main->maininvoiceID] = $this->invoice_m->get_order_by_invoice(array('maininvoiceID' => $main->maininvoiceID));
                    $main->invoice = $invoices[$main->maininvoiceID];
            ...
            $maininvoices = $this->maininvoice_m->get_maininvoice_with_studentrelation($schoolyearID);

            if(!empty($maininvoices)){
            foreach($maininvoices as $main)
                {
                    $invoices[$main->maininvoiceID] = $this->invoice_m->get_order_by_invoice(array('maininvoiceID' => $main->maininvoiceID));
                    $main->invoice = $invoices[$main->maininvoiceID];
                    ...
                    if(customCompute($invoiceitems[$maininvoice->maininvoiceID])) {
                        foreach ($invoiceitems[$maininvoice->maininvoiceID] as $invoiceitem) {
                            ...
                            if(isset($retArray['grandtotal'][$maininvoice->maininvoiceID])) {
                                ...
                                $retArray['grandtotal'][$maininvoice->maininvoiceID] = floatval($amount);
                            }
                            ...
                            if(isset($retArray['totalamount'][$maininvoice->maininvoiceID])) {
                                ...
                                $retArray['totalamount'][$maininvoice->maininvoiceID] = floatval($invoiceitem->amount);
                            }
                                ...
                                if(customCompute($paymentitems[$invoiceitem->invoiceID])) {
                                    foreach ($paymentitems[$invoiceitem->invoiceID] as $paymentitem) {
                                        if(isset($retArray['totalpayment'][$maininvoice->maininvoiceID])) {
                                            ...
                                            $retArray['totalpayment'][$maininvoice->maininvoiceID] = floatval($paymentitem->paymentamount);
                                        }
                                    }
                                }
                            }
                                ...
                                if(customCompute($weaverandfineitems[$invoiceitem->invoiceID])) {
                                    foreach ($weaverandfineitems[$invoiceitem->invoiceID] as $weaverandfineitem) {
                                        if(isset($retArray['totalweaver'][$maininvoice->maininvoiceID])) {
                                            ...
                                            $retArray['totalweaver'][$maininvoice->maininvoiceID] = floatval($weaverandfineitem->weaver);
                                        }
                                        ...
                                        if(isset($retArray['totalfine'][$maininvoice->maininvoiceID])) {
                                            ...
                                            $retArray['totalfine'][$maininvoice->maininvoiceID] = floatval($weaverandfineitem->fine);
                                        }
                                    }
                                }
                            }
                            ...
                            if(customCompute($paymentitems[$invoiceitem->invoiceID])) {
                                foreach ($paymentitems[$invoiceitem->invoiceID] as $paymentitem) {
                                    if(isset($retArray['totalpayment'])) {
                                        ...
                                        $retArray['totalpayment'] = floatval($paymentitem->paymentamount);
                                    }
                                }
                            }
                        }
                            ...
                            if(customCompute($weaverandfineitems[$invoiceitem->invoiceID])) {
                                foreach ($weaverandfineitems[$invoiceitem->invoiceID] as $weaverandfineitem) {
                                    if(isset($retArray['totalweaver'])) {
                                        ...
                                        $retArray['totalweaver'] = floatval($weaverandfineitem->weaver);
                                    }

                                    if(isset($retArray['totalfine'])) {
                                        $retArray['totalfine'] = ($retArray['totalfine'] + $weaverandfineitem->fine);
                                    } else {
                                        $retArray['totalfine'] = floatval($weaverandfineitem->fine);
                                    }
                                }
                            }
                        }
