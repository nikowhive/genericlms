...
class Student extends Api_Controller 
{
    ...
    public function index_get($id = null)
    {
        ...
        if($this->session->userdata('usertypeID') == 3 && $myProfile) {
            ...
            if((int)$id) {
                $this->retdata['students'] = $this->studentrelation_m->general_get_order_by_student(array('srclassesID' => $id, 'srschoolyearID' => $schoolyearID));
                if(customCompute($this->retdata['students'])) {
                    $sections = $this->section_m->general_get_order_by_section(array("classesID" => $id));
                    foreach ($sections as $key => $section) {
                        $this->retdata['allsection'][$section->sectionID] = $this->studentrelation_m->general_get_order_by_student(array('srclassesID' => $id, "srsectionID" => $section->sectionID, 'srschoolyearID' => $schoolyearID));
                    }
                } else {
    ...
    public function parent_get($id = null)
    {
        $myProfile = false;
        $schoolyearID = $this->session->userdata('defaultschoolyearID');
        if($this->session->userdata('usertypeID') == 3) {
        ...
        if($this->session->userdata('usertypeID') == 3 && $myProfile) {
            ...
            if((int)$id) {
                $this->retdata['students'] = $this->studentrelation_m->general_get_order_by_parent(array('srclassesID' => $id, 'srschoolyearID' => $schoolyearID));
                if(customCompute($this->retdata['students'])) {
                    $sections = $this->section_m->general_get_order_by_section(array("classesID" => $id));
                    foreach ($sections as $key => $section) {
                        $this->retdata['allsection'][$section->sectionID] = $this->studentrelation_m->general_get_order_by_parent(array('srclassesID' => $id, "srsectionID" => $section->sectionID, 'srschoolyearID' => $schoolyearID));
                    }
                } else {
                    $this->retdata['students'] = [];
                }
