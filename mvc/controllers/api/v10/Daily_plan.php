...
class Daily_plan extends Api_Controller
{
    ...
    protected function rules()
    {
        $rules = array(
            ...
            array(
                'field' => 'section_id',
                'label' => $this->lang->line("section_id"),
                'rules' => 'trim|required|xss_clean|max_length[128]',
            ),
            array(
                'field' => 'caption[]',
    ...
    public function index_get()
    {
        ...
        if($usertypeID == 3) {
            $student = $this->studentrelation_m->get_single_student(array('srstudentID' => $userID, 'srschoolyearID' => $schoolyearID), true);
            $filters['class_id'] = $student->classesID;
        }

        if($usertypeID == 4) {
            $studentID = $_GET['studentID'];
            $student = $this->studentrelation_m->get_single_student(array('srstudentID' => $studentID, 'srschoolyearID' => $schoolyearID), true);
        ...
        $this->retdata['dailys'] = $this->daily_plan_m->get_all_daily_plans_api($userID, $usertypeID,$filters);

        foreach ($this->retdata['dailys'] as $index => $v) {
            $user = $this->teacher_m->get_individual_teacher($v->user_id);
            $this->retdata['dailys'][$index]->created_by = $user->name;
            $this->retdata['dailys'][$index]->medias = $this->daily_plan_media_m->get_order_by_daily_plan_media(["daily_plan_id" => $v->id]);
        }
    ...
    public function add_post($subject_id = '')
    {
        if (($this->session->userdata('usertypeID') == 2)) {
            ...
            $subject = $this->courses_m->get_single_courses(['subject_id' => $subject_id]);
            $course = $subject->id;
            ...
            if ($_POST) {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $array = array(
                        ...
                        "title" => $_POST['title'],
                        "user_id" => $userID,
                        "user_type" => $this->retdata['usertype'],
                        "create_date" => date("Y-m-d H:i:s", strtotime($_POST["daily_date"].' '.date('H:i:s'))),
                        "schoolyearID"  => $this->session->userdata('defaultschoolyearID'),
                        "published"             => 1
                    );
                    ...
                    if ($daily_plan_id) {
                        ...
                        $students =  $this->student_m->get_students_from_section_id($array['sectionID']);
                        if(count($students) > 0){
                             $studentarray = [];
                        ...
                        $version = array(
                            ...
                            "user_id" => $this->retdata['usertypeID'],
                            "user_type" => $this->retdata['usertype'],
                            "finalized_id" => 1
                        );
                        ...
                        // for response
                        $this->retdata['daily_plan']  = $daily_pan = $this->daily_plan_m->get_single_daily_plan_detail($daily_plan_id);
                        $user = $this->teacher_m->get_individual_teacher($daily_pan->user_id);
                        $this->retdata['daily_plan']->created_by = $user->name;
                        $this->retdata['daily_plan']->medias = $this->daily_plan_media_m->get_order_by_daily_plan_media(["daily_plan_id" => $daily_pan->id]);
                        $this->retdata['versions'] = $this->daily_plan_version_m->get_order_by_daily_plan_version(["daily_plan_id" => $daily_plan_id]);
    ...
    public function edit_post($daily_id = '')
    {
        ...
        if ((int) $daily_id) {
            ...
            $this->retdata['usertype'] = $this->session->userdata('usertype');
                ...
                $check_daily = $this->daily_plan_m->get_daily_plan($daily_id);
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        $array1 = array(
                            ...
                            "teacherID" => $this->retdata['usertypeID'],
                        ...
                        if ($daily_plan_id) {
                            ...
                            // for response
                            $this->retdata['daily_plan']  = $daily_pan = $this->daily_plan_m->get_single_daily_plan_detail($daily_id);
                            $user = $this->teacher_m->get_individual_teacher($daily_pan->user_id);
                            $this->retdata['daily_plan']->created_by = $user->name;
                            $this->retdata['daily_plan']->medias = $this->daily_plan_media_m->get_order_by_daily_plan_media(["daily_plan_id" => $daily_pan->id]);
                            $this->retdata['versions'] = $this->daily_plan_version_m->get_order_by_daily_plan_version(["daily_plan_id" => $daily_id]);
    ...
    public function view_get($id = '')
    {
        ...
        if ((int) $id) {
                ...
                $this->retdata['dailys']  = $daily_pan = $this->daily_plan_m->get_single_daily_plan_detail($id);
                $user = $this->teacher_m->get_individual_teacher($daily_pan->user_id);
                $this->retdata['dailys']->created_by = $user->name;
                $this->retdata['dailys']->medias = $this->daily_plan_media_m->get_order_by_daily_plan_media(["daily_plan_id" => $daily_pan->id]);

                $this->retdata['versions'] = $this->daily_plan_version_m->get_order_by_daily_plan_version(["daily_plan_id" => $id]);
                foreach ($this->retdata['versions'] as $index1 => $v2) {
                    $this->retdata['versions'][$index1]->media = $this->daily_plan_media_m->get_order_by_daily_plan_media(["daily_plan_version_id" => $v2->id,]);
                }
    ...
    public function student_diary_get()
    {
    ...
    public function lesson_plans_get($subject_id = ''){
        ...
        if (($this->session->userdata('usertypeID') == 2)) {
            ...
            $this->retdata['lessons'] = $this->lesson_plan_m->get_order_by_lesson_plan(["course_id" => $course,'published' => 1]);
    ...
    public function sections_get($subject_id = ''){
        ...
        $course = $this->courses_m->get_single_courses(['subject_id' => $subject_id]);
    ...
    public function student_count_by_section_get($section_id = ''){

        if(!$section_id){
            $this->response([
                'status'    => false,
                'message'   => 'section id empty.',
                'data'      => [],
                
              ], REST_Controller::HTTP_BAD_REQUEST);
        }

        $this->retdata['students']  = count($this->student_m->get_students_from_section_id($section_id));
