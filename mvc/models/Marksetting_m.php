...
require_once 'Subjectmark_m.php';
...
class Marksetting_m extends MY_Model {
	...
	public function get_latest_marksetting($array=[]) {
		$this->db->select('*');
	...
	public function get_marksetting_with_marksettingrelation($array=[]) {
		...
		$this->db->order_by('marksetting.marksettingID ASC');
		$query = $this->db->get();
		return $query->result();
	}

	public function get_marksetting_with_marksettingrelation1($condition = []) {
		$this->db->select('*');
		$this->db->from('marksetting');
		$this->db->join('marksettingrelation', 'marksetting.marksettingID=marksettingrelation.marksettingID');
	...
	public function get_marksetting_markpercentages_add($array) {
		...
		$subjectmark_m = new Subjectmark_m;
		$subject_mark = $subjectmark_m->get_subjects_by_class_exam_and_subject_id(
			$array['classesID'],$array['subjectID'],$array['examID']);
		if(customCompute($subject_mark)){
			$finalmark = $subject_mark->fullmark;
		}	
