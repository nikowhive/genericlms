...
class Lmember_m extends MY_Model {
    ...
    public function get_single_lmember_with_student($array=NULL) {
        $this->db->select('lmember.lID, lmember.studentID, student.studentID, student.name, student.roll, student.classesID, student.sectionID, classes.classesID, classes.classes,  section.sectionID, section.section')
            ->from('lmember')
            ->join('student', 'student.studentID = lmember.studentID')
            ->join('classes', 'student.classesID = classes.classesID')
            ->join('section', 'student.sectionID = section.sectionID')
            ->where('lmember.lID', $array['lID']);
        $query = $this->db->get();
		return $query->row();
	}
