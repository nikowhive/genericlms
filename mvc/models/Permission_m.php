...
class Permission_m extends MY_Model {
	...
	public function delete_permission_relation_byid($id)
	{
		$this->db->where(array('permission_id' => $id));
	  	$this->db->delete('permission_relationships'); 
	  	return true;
	}

	public function general_get_single_permission($array=NULL) {
		$query = parent::get_single($array);
		return $query;
	}

	public function usertypeByPermissionID($permissionID){

		$this->db->select('permission_relationships.usertype_id');
		$this->db->from('permission_relationships');
		$this->db->where('permission_relationships.permission_id', $permissionID);
		$query = $this->db->get();
		return $query->result();		
	}
