...
class Invoice_m extends MY_Model {
	...
	public function get_book_invoice_by_student($data) {
		$this->db->select('*');
		$this->db->from('invoice');
		$this->db->where('invoice.schoolyearID',$data['schoolyearID']);
		$this->db->where('invoice.studentID',$data['studentID']);
		$this->db->where('invoice.feetypeID',$data['feetypeID']);
