...
class Studentparentteacher_m extends CI_Model {
    ...
    public function studentparentTeacher($array = NULL) {
		...
		if($usertypeID == 3) {
			...
			if(customCompute($studentrelationResult)) {
				...
				if(customCompute($array)) {
					$this->db->from('subjectteacher')->where(array('teacherID' => $array['teacherID']))->order_by('classesID');
					$subjectTeacherClassQuery = $this->db->get();
					$subjectTeacherClassResult = $subjectTeacherClassQuery->result();
				}
				
				$teacherMerged = (object) array_merge((array) $classResult, (array) $routineResult, (array) $subjectTeacherClassResult);
	...
	public function get_single_studentparent_teacher($array = NULL) {
		$teacherArray = $this->studentparentTeacher($array);
		if(customCompute($teacherArray)) {
			if(is_array($array)) {
