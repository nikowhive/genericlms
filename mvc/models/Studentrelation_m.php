...
class Studentrelation_m extends MY_Model {
	...
	private function userRelation() {
		...
		if($usertypeID == 2) {
			...
			$classMerged = (object) array_merge((array) $classResult , (array) $routineResult, (array) $subjectTeacherClassResult);
	...
	public function general_get_order_by_student_with_parent($arrays = [], $studentExtend = FALSE) {
		$arrays = $this->prefixLoad($arrays);
        $this->db->select('student.*, studentrelation.*, studentextend.*, parents.parentsID, parents.name as parent_name');
        $this->db->from('studentrelation');
		$this->db->join('student', 'student.studentID = studentrelation.srstudentID', 'LEFT');
	...
	public function general_get_order_by_parent($arrays = [], $studentExtend = FALSE) {
		$arrays = $this->prefixLoad($arrays);
        $this->db->select('student.*, studentrelation.*, studentextend.*, parents.parentsID, parents.name as parent_name');
        $this->db->from('studentrelation');
		$this->db->join('student', 'student.studentID = studentrelation.srstudentID', 'LEFT');
        ...
        $this->db->where('student.studentID !=', NULL);
        $this->db->where('student.parentID !=', 0);
		$this->db->where('student.active',1);
        $this->db->order_by('srroll asc');
        $query = $this->db->get();
        return $query->result();
	...
	public function get_register_no_student_by_limit($array=[])
	{
		$this->db->select('*');
        $this->db->from('studentrelation');
        // $this->db->join('student', 'student.studentID = studentrelation.srstudentID', 'LEFT');
		$this->db->where_in('studentrelation.srregisterNO',$array);
		$this->db->order_by('srregisterNO asc');
		// $this->db->limit(1);
		$query = $this->db->get();
        return $query->result();
    ...
    public function get_single_student($arrays = [], $studentExtend = FALSE,$optionalArray = []) {
    ...
    public function get_order_by_student($arrays = [], $studentExtend = FALSE,$optionalArray = []) {
		$this->_relation_array = $this->userRelation();
		if(!customCompute($this->_relation_array) && in_array($this->session->userdata('usertypeID'), $this->_user_role_array)) {
			return [];
		}
		...
		$arrays = $this->prefixLoad($arrays);
        $this->db->select('student.*, studentrelation.*, parents.parentsID, parents.name as parent_name, parents.phone as parent_phone,parents.email as pemail');
        $this->db->from('studentrelation');
        $this->db->join('student', 'student.studentID = studentrelation.srstudentID', 'LEFT');
		$this->db->join('parents', 'student.parentID = parents.parentsID', 'LEFT');
		...
		// if(isset($optionalArray['subjectID'])) {
		//  	$this->db->where('studentrelation.sroptionalsubjectID', $optionalArray['subjectID']);
		// 	$this->db->or_where('studentrelation.sranotheroptionalsubjectID', $optionalArray['subjectID']);
		// }

		if(isset($optionalArray['subjectID'])) {
            $opt = $optionalArray['subjectID'];
            $this->db->where("(studentrelation.sranotheroptionalsubjectID='$opt' OR studentrelation.sroptionalsubjectID='$opt')");
        }
	...
	public function get_order_by_all_student($arrays = [], $studentExtend = FALSE,$optionalArray = []) {
		$this->_relation_array = $this->userRelation();
		if(!customCompute($this->_relation_array) && in_array($this->session->userdata('usertypeID'), $this->_user_role_array)) {
			return [];
		}
	...
	function update_studentrelation_by_studentID($data, $id = NULL) {
        $this->db->update($this->_table_name, $data, "srstudentID = $id");
        return $id;
    }
