...
class Section_m extends MY_Model {
	...
	public function get_join_sections() {
		$this->db->select('section.*, teacher.teacherID, teacher.name');
		$this->db->from('section');
		$this->db->join('teacher', 'section.teacherID = teacher.teacherID', 'LEFT');
		$query = $this->db->get();
		return $query->result();
	...
	public function get_join_single_section() {
		$this->db->select('section.*, teacher.teacherID, teacher.name');
		$this->db->from('section');
		$this->db->join('teacher', 'section.teacherID = teacher.teacherID', 'LEFT');
		$query = $this->db->get();
	...
	public function getSectionByID($id){
		$this->db->select('*');
		$this->db->from('section');
		$this->db->where(array('sectionID'=>$id));
        $query = $this->db->get();
        return $query->row()->section;
	} 

	public function getSectionByTeacher($array=[]){
		$this->db->select('*');
		$this->db->from('section');
		$this->db->where(array('teacherID'=>$array['teacherID']));
		$this->db->where(array('classesID'=>$array['classesID']));
        $query = $this->db->get();
        return $query->result();
	}
}